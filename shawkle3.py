...
# http://code.google.com/p/shawkle/ - to check out: http://shawkle.googlecode.com/svn/trunk
...
def urlify(listofdatafiles, sedtxt, sedhtml, htmldir, cloud):
    ...
    # 2011-01-17: Problem with following: "htmldir" might be specified in the arguments with full pathname!
    dirhtml = str(os.getcwd()) + '/' + htmldir + '/'
    cloudlines = []
    if os.path.isfile(cloud):
        cloudfile = open(cloud, 'r')
        cloudlines = cloudfile.readlines()
        cloudfile.close()
    if not os.path.isdir(dirhtml):
        print 'Creating directory', repr(dirhtml)
        os.mkdir(dirhtml)
    else:
        try:
            shutil.rmtree(dirhtml)
            print 'Removing and re-creating', repr(dirhtml), '- directory for urlified files'
            os.mkdir(dirhtml)
        except:
            print 'Could not remove and re-create directory', repr(dirhtml), '- HTML files are therefore out of date'
            sys.exit()
